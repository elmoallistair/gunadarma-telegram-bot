...
def create_button(command, from_callback=False):
    ...
    if command in template.keys():
        for i, (text,url) in enumerate(template[command]):
            keyboard.append([InlineKeyboardButton(text=text, callback_data=f"{command}_callback", url=url)])
        return InlineKeyboardMarkup(keyboard)
    return None

def create_button_from_callback(callback_name):
    template = {
        "cuti_callback":            [("📝  Formulir Cuti Akademik", "https://baak.gunadarma.ac.id/public/file/Administrasi%20Akademik/F-CUTI%20New.doc")],
        "non_aktif_callback":       [("📝  Formulir Tidak Aktif Kuliah", "https://baak.gunadarma.ac.id/public/file/Administrasi%20Akademik/Formulir%20NonAktif.pdf")],
        "pindah_kelas_callback":    [("📝  Formulir Pindah Kelas", "https://baak.gunadarma.ac.id/public/file/Administrasi%20Akademik/F-PINKEL%20New.doc")],
        "pindah_jurusan_callback":  [("📝  Formulir Pindah Jurusan Fakultas Ekonomi", "https://baak.gunadarma.ac.id/public/file/Administrasi%20Akademik/pindah_jurusan_ekonomi.pdf"),
                                     ("📝  Formulir Pindah Jurusan Fakultas Ilkom", "https://baak.gunadarma.ac.id/public/file/Administrasi%20Akademik/pindah_jurusan_ilkom.pdf"),
                                     ("📝  Formulir Pindah Jurusan Fakultas Teknologi Industri]", "https://baak.gunadarma.ac.id/public/file/Administrasi%20Akademik/pindah_jurusan_ti.pdf")]
    }
